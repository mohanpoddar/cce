...
def get_latest_rsync_log():
    return latest_matching_file([
        "/home/cce/logs/rsync/rsync_cron_*",
        "/home/cce/logs/rsync/rsync_ransomware_protect_*.log",
    ])

def get_latest_rsync_actual_command_log():
    return latest_matching_file([
        "/home/cce/logs/rsync/rsync_in_backup_location_latest_*",
        "/home/cce/logs/rsync/rsync_ransomware_protect_*.log",
    ])
...
backup_status = os.environ.get("BACKUP_STATUS", "").upper()
if backup_status == "FAILED":
    error = True
    print("Return Error:", error)
elif not latest_log_cron_path or not latest_rsync_actual_log_path:
    error = True
    print("Return Error:", error)
elif latest_log_cron_path != latest_rsync_actual_log_path and latest_log_cron_path.split('_')[-1] != latest_rsync_actual_log_path.split('_')[-1]:
    error = True
    print ("Return Error:", error)

if not latest_log_cron_path:
    latest_log_cron_path = "/dev/null"
if not latest_rsync_actual_log_path:
    latest_rsync_actual_log_path = "/dev/null"
